...
public class PerceptronVisualizer extends PApplet {
	...
	public void setup() {
		...
		data = DataReader.createDataSet("iris.data", 0, headers);

		data.removeType(DataPoint.iris_versicolor);
	...
	private void drawPredictions() {
		int n = dataViz.getN();
		for (int i = 0; i < n; i++) {
			DataPoint p = data.getData().get(i);
			float[] input = {p.getData(0), p.getData(1)};
			
			int guess = nn.guess(input);
			int color = (guess == p.getLabel()) ? color(0, 255, 0) : color(255, 0, 0);
	...
	private void drawPerceptron() {
		...
		String label = DataPoint.getNameFor(guess);

		textSize(40);
		int color = (guess == current.getLabel()) ? color(0, 180, 0) : color(255, 0, 0);
		fill(color);
		text(guess + " : " + label, centerX - 60, centerY + 200);
	}
	...
	public void keyReleased() {
		...
		if (key == 't') {
			...
			nn.train(input, current.getLabel());
		}
	}
}